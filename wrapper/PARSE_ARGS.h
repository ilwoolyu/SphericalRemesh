...
void PARSE_ARGS(int argc, char **argv)
{
	...
	CLI::App app(desc);
	app.add_flag("--quiet", quite, "Do not print outputs");
	...
	app.add_option("--ftype", ftype, "Specify data type - output will be saved as a binary format (dat)")->check(CLI::IsMember({"int", "int16", "int32", "float","double"}, CLI::ignore_case, CLI::ignore_underscore))->group("Outputs");
